...
public final class RateIntGroupingAggregatorFunction extends AbstractRateGroupingFunction implements GroupingAggregatorFunction {
    ...
    // [3] firstValue (widened int value stored as long at firstTs)
    // [4] lastTs (raw long, most-recent timestamp)
    // [5] lastValue (widened int value stored as long at lastTs); 0L for DELTA (sentinel)
    //
    ...
    private void addRawInput(
        ...
        for (int p = 0; p < positionCount; p++) {
            ...
            for (int g = groupStart; g < groupEnd; g++) {
                ...
                final int value = valueBlock.getInt(valueBlock.getFirstValueIndex(valuePosition));
    ...
    private void addSubRange(
        ...
        if (temporality == Temporality.CUMULATIVE) {
            ...
            for (int pos = from; pos < to; pos++) {
                appendDeltaValue(group, timestampVector.getLong(pos), valueVector.getInt(pos));
            }
        }
    }
    ...
    private void addSubRange(
        ...
        if (temporality == Temporality.CUMULATIVE) {
            ...
            for (int pos = from; pos < to; pos++) {
                ...
                appendDeltaValue(group, timestampVector.getLong(pos), valueBlock.getInt(pos));
            }
        }
    }
    ...
    void flushGroup(int groupId, IntRawBuffer buffer, FlushQueue flushQueue) {
    ...
    private double firstValue(int groupId) {
        ...
        return (int) state.get((long) groupId * STRIDE + 3);
    }
    ...
    private double lastValue(int groupId) {
        ...
        // DELTA: lane 5 is always 0L = (int)0 = 0 (sentinel; reset accounting goes through resets field)
        // INLINE: stored as widened int in lane 5
        return (int) state.get((long) groupId * STRIDE + 5);
    }
    ...
    private void appendInterval(
        ...
        if (kind == 0L) {
            ...
            int oldId = intervalBuffer.appendInterval(
                state.get(base + 4),
                (int) state.get(base + 5),
                state.get(base + 2),
                (int) state.get(base + 3)
            );
    ...
    private void appendDeltaValue(int groupId, long timestamp, int value) {
        ...
        if (prevSamples == 0) {
            ...
            state.set(base + 3, (long) value);
            state.set(base + 4, timestamp);
            // lane 5 stays 0L = (int)0 = 0 (sentinel lastValue for delta)
        } else {
    ...
    private void appendIntervalsFromBlocks(int groupId, LongBlock ts, IntBlock vs, int position) {
    ...
    private void writeIntervalsToBlocks(int groupId, LongBlock.Builder timestamps, IntBlock.Builder values) {
        ...
        if (k == 1L) {
            ...
            values.appendInt(0);                          // lastValue sentinel
            values.appendInt((int) state.get(base + 3)); // firstValue = deltaFirstValue
        } else if (k == 2L) {
            // INLINE: single cumulative interval
            timestamps.appendLong(state.get(base + 4));
            timestamps.appendLong(state.get(base + 2));
            values.appendInt((int) state.get(base + 5));
            values.appendInt((int) state.get(base + 3));
        } else {
            ...
            for (int i = 0; i < sz; i++) {
                ...
                values.appendInt(intervalBuffer.lastValue(id));
                values.appendInt(intervalBuffer.firstValue(id));
            }
        }
